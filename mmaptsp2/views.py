...
def addmmap(request, profile_id):    
    ...
    if (d_o_b >= 18 & d_o_b <= 64):
        mmaptype = "18-64"

    elif (d_o_b >= 65 & d_o_b <= 69):
        mmaptype = "65-69"
...
def processaddmmap(request):
    ...
    mmap = Mmap.objects.create(clientid=clientid, mmap_name=mmap_name, mmap_sex=mmap_sex, mmap_age=mmap_age, mmap_branch=mmap_branch,
            ...
            clientimg=clientimg, mmapbcode=branchcode, mmapservicecentercontrolno=sscnservicecentercontrolno,
    ...
    getidcount = Mmap.objects.only('id').count()
...
def mmapedit(request, mmap_id):
    try:
        ...
        beneficiary = Beneficiary.objects.get(pk=mmap_id, client_cycle=mmap.client_cycle)       
    except Client.DoesNotExist:
        raise Http404("Profile does not exist")
...
# def load30days(request):

#     days30 = datetime.date.today() + datetime.timedelta(-30)
#     mmap = Mmap.objects.filter(mmap_expired__range=[datetime.date.today(), days30]).order_by('-mmap_expired')
